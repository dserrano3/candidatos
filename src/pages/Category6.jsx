...
function Category6() {
  const [candidates, setCandidates] = useState({
    ...
    fajardo: { summary: 'Cargando...', sources: [] },
    lopez: { summary: 'Cargando...', sources: [] }
  })
  ...
  useEffect(() => {
    const load = async () => {
      const data = await loadAllCandidates(getEnergia)
      setCandidates(data)
    }
    load()
  }, [])
  ...
  return (
    <div className="p-[30px] bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] max-[900px]:p-6 max-md:p-5 max-md:rounded-xl max-[480px]:p-4 max-[480px]:rounded-[10px]">
      <h1 className="text-[#1a1a2e] mb-2.5 text-3xl font-bold max-md:text-2xl max-[480px]:text-[1.35rem]">Energía y Cambio Climático</h1>
      <p className="text-gray-500 mb-5 text-base">Resumen de las propuestas de energía y cambio climático de los candidatos.</p>
