...
import { loadAllCandidates, getVicepresidente } from '../utils/helpers'
import Navigation from '../components/Navigation'
import Footer from '../components/Footer'

const CANDIDATE_INFO = [
  { key: 'cepeda', name: 'Cepeda', image: '/cepeda.jpg', vpImage: '/aida.jpg', vpName: 'Aída Quilcué' },
  { key: 'espriella', name: 'Espriella', image: '/espriella.jpg', vpImage: '/jose.webp', vpName: 'José Manuel Restrepo' },
]
...
function Photo({ src, alt, label, size = 'lg' }) {
  const dim = size === 'lg'
    ? 'w-[88px] h-[88px] max-[900px]:w-[76px] max-[900px]:h-[76px]'
    : 'w-[72px] h-[72px] max-[900px]:w-[62px] max-[900px]:h-[62px]'
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className={`${dim} bg-gradient-to-br from-slate-200 to-slate-300 rounded-full shrink-0 overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.1)] border-[3px] border-white`}>
        <img src={src} alt={alt} className="w-full h-full object-cover" />
      </div>
      {label && (
        <span className="text-[0.7rem] font-semibold text-gray-400 uppercase tracking-wide text-center leading-tight max-w-[90px]">
          {label}
        </span>
      )}
    </div>
...
function Category7() {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await loadAllCandidates(getVicepresidente)
      setCandidates(data)
    }
    load()
  }, [])
  ...
  return (
    <div className="p-[30px] bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] max-[900px]:p-6 max-md:p-5 max-md:rounded-xl max-[480px]:p-4 max-[480px]:rounded-[10px]">
      <h1 className="text-[#1a1a2e] mb-2.5 text-3xl font-bold max-md:text-2xl max-[480px]:text-[1.35rem]">Vicepresidente</h1>
      <p className="text-gray-500 mb-5 text-base">Resumen del vicepresidente escogido por cada candidato.</p>
      ...
      <div className="mt-10 flex flex-col gap-6 max-md:mt-6 max-md:gap-4">
        {shuffledCandidates.filter(c => candidates[c.key] !== null).map((candidate) => (
          ...
          >
            {/* Duo photo block */}
            <div className="flex items-start gap-2 shrink-0 max-md:justify-center">
              <Photo src={candidate.image} alt={candidate.name} label={null} size="lg" />
<Photo src={candidate.vpImage} alt={candidate.vpName} label={candidate.vpName} size="sm" />
            </div>
